...
export default function ShufflePanel({ friends, onShuffle }: ShufflePanelProps) {
  ...
  async function handleShuffle() {
    ...
    if (selectedIds.length < 2) {
      setError('Seleccioná al menos 2 participantes')
      return
    }
    setLoading(true)
    try {
  ...
  return (
    <div ref={panelRef} className="relative bg-warm-900/60 border border-warm-700/40 rounded-2xl p-6 sm:p-8 animate-fade-in-up overflow-hidden">
      {celebrating && (
      ...
      <div className="mb-5">
        <p className="text-sm text-cream-200/60 mb-3">Seleccioná los participantes:</p>
        ...
        {!friends.length && <p className="text-cream-200/30 text-sm mt-3">Agregá amigos primero</p>}
      </div>

      <div className="flex items-center gap-4 mb-5">
        <label className="text-sm text-cream-200/60">Monto sugerido ($):</label>
      ...
      {error && (
        <p className="text-crimson-400 text-sm bg-crimson-600/10 rounded-lg py-2 px-4 mb-4 inline-block">{error}</p>
      )}
      ...
      {result && (
        <div className="mt-6 p-5 bg-gradient-to-br from-gold-400/10 to-transparent border border-gold-400/20 rounded-xl animate-scale-in">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-gold-300 font-display text-base">¡Sorteo realizado con éxito!</p>
          </div>
          <div className="flex gap-6 text-sm mt-3">
            <span className="text-cream-200/60">Participantes: <strong className="text-cream-100">{result.participantCount}</strong></span>
            <span className="text-cream-200/60">Monto: <strong className="text-gold-300">${result.giftAmount.toFixed(2)}</strong></span>
          </div>
        </div>
      )}
    </div>
